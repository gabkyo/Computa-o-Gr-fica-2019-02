...
void drawRetangle2D(Paralelepipedo r)
{
    cor(r.color);
    glBegin(GL_POLYGON);
    {
        glVertex2d(r.cx - r.largura / 2, r.cy + r.comprimento / 2);
        glVertex2d(r.cx - r.largura / 2, r.cy - r.comprimento / 2);
        glVertex2d(r.cx + r.largura / 2, r.cy - r.comprimento / 2);
        glVertex2d(r.cx + r.largura / 2, r.cy + r.comprimento / 2);
    }
...
void drawNave2D(Nave *nave)
{
    ...
    glScaled(0.33, 1, 1);
    glTranslated(-nave->hitbox.cx, -nave->hitbox.cy, 0);
    paral_temp.comprimento = nave->hitbox.raio / 3;
    paral_temp.largura = nave->hitbox.raio * 2 / 5;
    drawCircle2D(nave->hitbox);
...
void addTiroInimigo(Nave *a)
{
    ...
    temp.tiro.cz = a->hitbox.cz + a->hitbox.raio * sin(a->phi) + a->hitbox.raio * 0.5 * sin(a->phi_canhao);
...
void addTiro()
{
    ...
    temp.phi = jogador.phi_canhao + jogador.phi;
    ...
    temp.tiro.cz = jogador.hitbox.cz + jogador.hitbox.raio * sin(jogador.phi) + jogador.hitbox.raio * 0.5 * sin(jogador.phi_canhao);
...
void mov_inimigo(int param)
{
    ...
    for (auto j = inimigos.begin(); j < inimigos.end(); ++j)
    {
        if (j->enabled)
        {
            j->angulo +=((rand()%3)-1)*15;
            if(j->angulo<0){
                j->angulo+=360;
            }else if(j->angulo>360){
                j->angulo-=360;
            }
        }
    }
